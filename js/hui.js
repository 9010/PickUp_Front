...
var hui = (function(selector){
	...
	_hui.alert_without_back = function(msg, btnName, callBack){
		hui.dialogCallBack = callBack;
		if(!btnName){btnName = '确定';}
		hui.dialogBase();
		hui.dialogDom.innerHTML = '<div id="hui-dialog-in"><div id="hui-dialog-msg">'+msg+'</div><div id="hui-dialog-btn-line">'+btnName+'</div></div>';
	...
	_hui.actionSheet = function(menus, cancel, callBack){
		...
		hui(huiActionSheet).find('li').click(function(){
			...
			if(menus[this.index] == "添加家长"){
				window.location.href='addParent.html';
			}
			else if(menus[this.index] == "添加子女"){
				window.location.href='addChild.html';
			}
			else if(menus[this.index] == "添加临时委托人"){
				window.location.href='temp.html';
			}
			else if(menus[this.index] == "添加学生"){
				window.location.href='addStudent.html';
			}
			else if(menus[this.index] == "添加事项"){
				window.location.href='addMatter.html';
			}
			//callBack(this); 
